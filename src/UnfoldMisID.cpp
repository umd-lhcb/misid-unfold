...
string getHistoEffName(const string& prefix, const string& ptcl1,
  ...
  return ""s + prefix + "__" + getHistoEffName(ptcl1, ptcl2, descr1, descr2) + skim_suffix;
}
...
void unfold(vStr& prefix, vStr& ptcls, vStr& skims, bool& ctrlSample, vector<int>& nbins, F1& histoInGetter,
        ...
        for (auto& pref : prefix) {
          for (const auto& skim : skims) {
            ...
            for (int idxTag = 0; idxTag != totSize; idxTag++) {
              ...
              auto name =
                  getHistoEffName(pref, ptcls[idxTag], "Mu", "Tag", "Tag", skim);
              auto histo = histoOutGetter(name);
...
template <typename F1, typename F2>
void unfoldDryRun(vStr& prefix, vStr& ptcls, vStr& skims, bool& ctrlSample, vFltFlt& binnings,
...
int main(int argc, char** argv) {
  ...
  if (parsedArgs["dryRun"].as<bool>()) {
    unfoldDryRun(prefix, ptclList, skims, ctrlSample, histoBinSpec, histoBinSize, histoInGetter,
                 histoOutGetter);
    return 0;
  }
  ...
  unfold(prefix, ptclList, skims, ctrlSample, histoBinSize, histoInGetter, histoOutGetter, debug,
         numOfIter);
