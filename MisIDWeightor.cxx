...
const TString pidfile_name = "myPid.root";
const bool verbose = false;
const bool very_verbose = false;

const TString CutString =
    "Jpsi_L0DiMuonDecision_TOS==1 && (Jpsi_Hlt1DiMuonHighMassDecision_TOS==1 "
    "|| Jpsi_Hlt1DiMuonLowMassDecision_TOS==1) && "
    "Jpsi_Hlt2DiMuonDetachedJPsiDecision_TOS==1 && Bc_ISOLATION_BDT<0.2 && "
    "Bc_MM<=6400 && Bc_MM>=3203 && Bc_ENDVERTEX_CHI2<25 && Bc_DOCA<0.15 && "
    "BachMu_IPCHI2_OWNPV>4.8 && MuM_IPCHI2_OWNPV>4 && MuP_IPCHI2_OWNPV>4 && "
    "Jpsi_MM<=3150 && Jpsi_MM>=3040 && BachMu_P>3000 && BachMu_P<100000 && "
    "nTracks<600 && BachMu_PT>750 && BachMu_isMuon==0";

const TString
    PiCutString = "(BachMu_ProbNNp<0.1)||!(BachMu_PIDK<1)||!(BachMu_ProbNNk>0."
                  "1)&&!(BachMu_PIDmu>2)&&!(BachMu_ProbNNe>0.1)&&!(BachMu_"
                  "ProbNNghost>0.2)"; // wrong & not used -- whatever fails all
                                      // other cuts is sorted as pi
const TString PCutString =
    "BachMu_ProbNNp>0.1&&BachMu_PIDK<1&&!(BachMu_ProbNNk>0.1)&&!(BachMu_PIDmu>"
    "2)&&!(BachMu_ProbNNe>0.1)&&!(BachMu_ProbNNghost>0.2)";
const TString KCutString = "BachMu_ProbNNk>0.1&&!(BachMu_PIDmu>2)&&!(BachMu_"
                           "ProbNNe>0.1)&&!(BachMu_ProbNNghost>0.2)";
const TString MuCutString =
    "BachMu_PIDmu>2&&!(BachMu_ProbNNe>0.1)&&!(BachMu_ProbNNghost>0.2)";
...
using CacheType =
    std::tuple<Double_t, Double_t, std::array<Double_t, numClasses>>;
...
public:
  ...
  MisIDFunction(int cuttype = 0)
      : cache(new std::map<Int_t, CacheType>()), pidfile(pidfile_name) {
  ...
  Double_t operator()(Double_t *x, Double_t *ps = 0) {
    ...
    // std::cout << HadronHypo << " - " << useNikhefPriors << " - " << PidBin <<
    // std::endl;
    ...
    if (it == cache->end()) {
      ...
      // CALCULATE VALUE
      std::cout << "Calculating value for bin " << PidBin << " ... "
                << std::endl;
      Double_t avgVal = 0.0;
      ...
      Double_t valbreak_umd[numClasses][numClasses] = {
          0}; // first index is hat category; second is weight for unfolded type
      Double_t val_hyb[numClasses] = {0};
      Double_t valsq_hyb[numClasses] = {0};
      Double_t valerr_hyb[numClasses] = {0};
      Double_t valbreak_hyb[numClasses][numClasses] = {
          0}; // first index is hat category; second is weight for unfolded type

      if (!HistoMatrix[0][0]->IsBinUnderflow(PidBin) &&
          !HistoMatrix[0][0]->IsBinOverflow(PidBin)) {
        ...
        if (totNum > 0) {
          ...
          Double_t measured_vector_total[numClasses] = {piNum, kNum, pNum,
                                                        muNum, eNum, uNum};
          ...
          TH2D *response_hist;
          measured_hist = new TH1D(TString::Format("measured_hist_%d", counter),
                                   TString::Format("measured_hist_%d", counter),
                                   numClasses, 0, numClasses);
          response_truth =
              new TH1D(TString::Format("response_truth_%d", counter),
                       TString::Format("response_truth_%d", counter),
                       numClasses, 0, numClasses);
          response_hist =
              new TH2D(TString::Format("response_hist_%d", counter),
                       TString::Format("response_hist_%d", counter), numClasses,
                       0, numClasses, numClasses, 0, numClasses);
          ++counter;
          ...
          for (int toy = 0; toy < numToys; toy++) {
            ...
            if (verbose)
              std::cout << "H -> H RESPONSE MATRIX:" << std::endl;
            for (int row = 0; row < numClasses; row++) {
              ...
              for (int col = 0; col < numClasses; col++) {
                response_hist->SetBinContent(row + 1, col + 1,
                                             response_matrix[row][col]);
                if (verbose)
                  std::cout << std::setw(10) << std::setprecision(4)
                            << response_hist->GetBinContent(row + 1, col + 1)
                            << " | ";
              }
              if (verbose)
                std::cout << std::endl;
            ...
            for (int i = 0; i < numClasses; i++) {
              ...
              if (verbose)
                std::cout << measured_vector[i] << " -> " << response_vector[i]
                          << " +/-" << reco->GetBinError(i + 1) << std::endl;
            }
            ...
            for (int i = 0; i < numClasses; i++) {
              ...
              if (verbose)
                std::cout << particle_name[i] << ": " << prior_probs_umd[i]
                          << " OR " << prior_probs_hyb[i] << std::endl;
            }
            ...
            for (int i = 0; i < numClasses; i++) {
              ...
              for (int j = 0; j < numClasses; j++) {
                posterior_probs_umd[i] +=
                    response_matrix[i][j] * prior_probs_umd[j];
                posterior_probs_hyb[i] +=
                    response_matrix[i][j] * prior_probs_hyb[j];
              }
              if (verbose)
                std::cout << particle_name[i] << ": " << posterior_probs_umd[i]
                          << " OR " << posterior_probs_hyb[i] << std::endl;
            }
            ...
            for (int i = 0; i < numClasses; i++) {
              for (int j = 0; j < numClasses; j++) {
                inverted_probs_umd[i][j] = response_matrix[j][i] *
                                           prior_probs_umd[i] /
                                           posterior_probs_umd[j];
                inverted_probs_hyb[i][j] = response_matrix[j][i] *
                                           prior_probs_hyb[i] /
                                           posterior_probs_hyb[j];
              }
            }
            if (verbose) {
              std::cout << "\n\n INVERTED PROBS --UMD :\n";
            ...
            for (int toy2 = 0; toy2 < numToys; toy2++) {
              ...
              for (int i = 0; i < numClasses; i++) {
                ...
                if (very_verbose)
                  std::cout << particle_name[i] << " FAKE RATE : ";
                if (very_verbose)
                  std::cout << eff_vector[i] << std::endl;
              }
              ...
              for (int j = 0; j < numClasses; j++) {
                for (int i = 0; i < numClasses; i++) {
                  valMatrix_umd[j][i] =
                      inverted_probs_umd[i][j] * eff_vector[i];
                  valMatrix_hyb[j][i] =
                      inverted_probs_hyb[i][j] * eff_vector[i];
                  valVector_umd[j] += valMatrix_umd[j][i];
                  valVector_hyb[j] += valMatrix_hyb[j][i];
                }
                if (very_verbose)
                  std::cout << particle_name[j] << ": " << valVector_umd[j]
                            << " OR " << valVector_hyb[j] << std::endl;
              }
          ...
          for (int i = 0; i < numClasses; ++i) {
            std::cout << particle_name[i] << ":  " << val_umd[i] << " +/- "
                      << valerr_umd[i] << " OR " << val_hyb[i] << " +/- "
                      << valerr_hyb[i] << std::endl;
          }
      ...
      for (int i = 0; i < numClasses; ++i) {
        ...
        cache->insert(
            CacheEntry(i + 100 * PidBin,
                       std::make_tuple(val_umd[i], valerr_umd[i], break_umd)));
        cache->insert(
            CacheEntry(i + 10 + 100 * PidBin,
                       std::make_tuple(val_hyb[i], valerr_hyb[i], break_hyb)));
      }
      std::array<Double_t, numClasses> aempty{};
      cache->insert(
          CacheEntry(numClasses + 100 * PidBin,
                     std::make_tuple(avgVal, avgValErr, aempty))); // nikhef
      cache->insert(
          CacheEntry(numClasses + 10 + 100 * PidBin,
                     std::make_tuple(avgVal, avgValErr, aempty))); // nikhef
    }
  ...
  void printAggregateResults() {
    ...
    for (int i = 0; i < numClasses; i++) {
      std::cout << particle_name[i] << ": ";
      std::cout << sum_measured_vector[i] << " -> " << sum_response_vector[i]
                << " +/-" << sqrt(sum_response_vector_var[i]) << std::endl;
    ...
    for (int i = 0; i < numClasses; i++) {
      ...
      std::cout << sum_measured_vector[i] / sum_measured << " -> "
                << sum_response_vector[i] / sum_response << " +/-"
                << sqrt(sum_response_vector_var[i]) / sum_response << std::endl;
    }
  }
...
void MisIDFunction::createCountCache(TTree *tree) {
  ...
  // TTreeReaderValue<Int_t> myHadronHypo(reader,"HadronHypo"); // built in
  // HadronHypo is out of date; this will be worth fixing
  // TTreeReaderValue<Bool_t>
  // myHlt2(reader,"Jpsi_Hlt2DiMuonDetachedJPsiDecision_TOS");
  ...
  TTreeFormula *EtaFormula =
      new TTreeFormula("BachMu_ETA", "TMath::ATanH(BachMu_PZ/BachMu_P)", tree);
  ...
  for (Long64_t entry = 0; entry < nentries; ++entry) {
    ...
    if (Cut->EvalInstance()) {
      ...
      if (it == MisIDFunction::countCache->end()) {
        MisIDFunction::countCache->insert(
            std::pair<Int_t, Species>(PidBin, sp));
      } else {
        if (hh == 1) // New PID
          it->second.kNum += 1.0;
        else if (hh == 2)
...
void MisIDWeightor() {
  ...
  TString oldfilename = "/afs/cern.ch/user/z/ziyang/my_eos/RJpsi/tuples/"
                        "2016_MisID_Data_MD_1-4k_MICRO.root";
  oldtree->Add(oldfilename.Data());

  TString newfilename = "/afs/cern.ch/user/z/ziyang/my_eos/RJpsi/tuples/"
                        "2016_MisID_Data_MD_1-4k_unfold.root";
  ...
  for (Int_t i = 0; i < numClasses; ++i) {
    PIDWfunc_breakdown[i] = new TF1("PIDWfunc_" + particle_name[i], fobj, 0,
                                    10000, 5, "MisIDFunction");
  ...
  if (kTRUE) {
    ...
    Int_t MISIDTYPE;
    TBranch *b_MISIDTYPE =
        newtree->Branch("MISIDTYPE", &MISIDTYPE, "MISIDTYPE/I");
    ...
    auto getPidBin = [&]() {
      PidBin = bingetter->FindFixBin(
          BachMu_P, TMath::ATanH(BachMu_PZ / BachMu_P), nTracks);
    };
    ...
    auto compute = [&]() {
      ...
      /// cout << PIDW
      /// <<","<<PIDWVar<<","<<PIDW_PI<<","<<PIDW_K<<","<<PIDW_P<<","<<PIDW_E<<","<<PIDW_MU<<","<<PIDW_U<<endl;
    };
