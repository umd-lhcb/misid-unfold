# Author: Yipeng Sun, Alex Fernez, Lucas Meyer Garcia
...
build-rdx-true-to-tag-2016-glacier:
	...
	@mkdir -p $(OUT_DIR)
	./scripts/pidcalib_wrapper.py -c $(YML_FILE) -o $(OUT_DIR) -y 2016 \
		-m glacier -u run1rdx 2>&1 | tee $(OUT_DIR)/stdout.log

build-rdx-true-to-tag-2017-glacier:
	$(eval OUT_DIR := $(GENPATH)/rdx-$(TIME_STAMP)-true_to_tag_glacier-2017)
	@mkdir -p $(OUT_DIR)
	./scripts/pidcalib_wrapper.py -c $(YML_FILE) -o $(OUT_DIR) -y 2017 \
		-m glacier -u run1rdx 2>&1 | tee $(OUT_DIR)/stdout.log

build-rdx-true-to-tag-2018-glacier:
	$(eval OUT_DIR := $(GENPATH)/rdx-$(TIME_STAMP)-true_to_tag_glacier-2018)
	@mkdir -p $(OUT_DIR)
	./scripts/pidcalib_wrapper.py -c $(YML_FILE) -o $(OUT_DIR) -y 2018 \
		-m glacier -u run1rdx 2>&1 | tee $(OUT_DIR)/stdout.log
...
build-rdx-true-to-tag-2016-glacier-run2ang:
	...
	@mkdir -p $(OUT_DIR)
	./scripts/pidcalib_wrapper.py -c $(YML_FILE_RUN2ANG) -o $(OUT_DIR) -y 2016 \
		-m glacier -u run2ang 2>&1 | tee $(OUT_DIR)/stdout.log

build-rdx-true-to-tag-2017-glacier-run2ang:
	$(eval OUT_DIR := $(GENPATH)/rdx-$(TIME_STAMP)-true_to_tag_glacier_run2ang-2017)
	@mkdir -p $(OUT_DIR)
	./scripts/pidcalib_wrapper.py -c $(YML_FILE_RUN2ANG) -o $(OUT_DIR) -y 2017 \
		-m glacier -u run2ang 2>&1 | tee $(OUT_DIR)/stdout.log

build-rdx-true-to-tag-2018-glacier-run2ang:
	$(eval OUT_DIR := $(GENPATH)/rdx-$(TIME_STAMP)-true_to_tag_glacier_run2ang-2018)
	@mkdir -p $(OUT_DIR)
	./scripts/pidcalib_wrapper.py -c $(YML_FILE_RUN2ANG) -o $(OUT_DIR) -y 2018 \
		-m glacier -u run2ang 2>&1 | tee $(OUT_DIR)/stdout.log
...
build-rdx-true-to-tag-2016-lxplus:
	...
	@mkdir -p $(OUT_DIR)
	./scripts/pidcalib_wrapper.py -c $(YML_FILE) -o $(OUT_DIR) -y 2016 \
		-m lxplus 2>&1 | tee $(OUT_DIR)/stdout.log
...
test-pidcalib2-wrapper-glacier:
	./scripts/pidcalib_wrapper.py -c $(YML_FILE) -o $(GENPATH) --dry-run \
		-m glacier -u run1rdx
...
build-rdx-true-to-tag-2016-local:
	...
	./scripts/build_histo_eff.py -c $(YML_FILE) -o $(OUT_DIR) -y 2016 \
		-u run1rdx 2>&1 | tee $(OUT_DIR)/stdout.log

build-rdx-true-to-tag-2017-local:
	$(eval OUT_DIR := $(GENPATH)/rdx-$(TIME_STAMP)-true_to_tag_local-2017)
	@mkdir -p $(OUT_DIR)
	./scripts/build_histo_eff.py -c $(YML_FILE) -o $(OUT_DIR) -y 2017 \
		-u run1rdx 2>&1 | tee $(OUT_DIR)/stdout.log

build-rdx-true-to-tag-2018-local:
	$(eval OUT_DIR := $(GENPATH)/rdx-$(TIME_STAMP)-true_to_tag_local-2018)
	@mkdir -p $(OUT_DIR)
	./scripts/build_histo_eff.py -c $(YML_FILE) -o $(OUT_DIR) -y 2018 \
		-u run1rdx 2>&1 | tee $(OUT_DIR)/stdout.log
...
build-rdx-true-to-tag-2016-local-run2ang:
	...
	@mkdir -p $(OUT_DIR)
	./scripts/build_histo_eff.py -c $(YML_FILE_RUN2ANG) -o $(OUT_DIR) -y 2016 \
		-u run2ang 2>&1 | tee $(OUT_DIR)/stdout.log

build-rdx-true-to-tag-2017-local-run2ang:
	$(eval OUT_DIR := $(GENPATH)/rdx-$(TIME_STAMP)-true_to_tag_local_run2ang-2017)
	@mkdir -p $(OUT_DIR)
	./scripts/build_histo_eff.py -c $(YML_FILE_RUN2ANG) -o $(OUT_DIR) -y 2017 \
		-u run2ang 2>&1 | tee $(OUT_DIR)/stdout.log

build-rdx-true-to-tag-2018-local-run2ang:
	$(eval OUT_DIR := $(GENPATH)/rdx-$(TIME_STAMP)-true_to_tag_local_run2ang-2018)
	@mkdir -p $(OUT_DIR)
	./scripts/build_histo_eff.py -c $(YML_FILE_RUN2ANG) -o $(OUT_DIR) -y 2018 \
		-u run2ang 2>&1 | tee $(OUT_DIR)/stdout.log
...
build-d0-bkg-decays: $(BINPATH)/d0BkgDecays
	...
	@mkdir -p $(OUT_DIR)
	$< -o $(OUT_DIR) -c $(YML_FILE) -f d0_bkg_constraints.yml \
...
build-d0-bkg-decays-run2ang: $(BINPATH)/d0BkgDecays
	...
	@mkdir -p $(OUT_DIR)
	$< -o $(OUT_DIR) -c $(YML_FILE_RUN2ANG) -f d0_bkg_constraints_run2ang.yml \
		--run2ang 2>&1 | tee $(OUT_DIR)/stdout.log
...
build-rdx-misid-mc-corrections-2016: $(BINPATH)/GetMisIDCorrections
	...
	$< -o $(OUT_DIR) -c $(YML_FILE) -b $(YML_D0_BKG) -s $(YML_PIDCALIB_EFFS) \
		-y 2016 2>&1 | tee $(OUT_DIR)/stdout.log
...
build-rdx-misid-mc-corrections-2017: $(BINPATH)/GetMisIDCorrections
	...
	@mkdir -p $(OUT_DIR)/figs/fits
	$< -o $(OUT_DIR) -c $(YML_FILE) -b $(YML_D0_BKG) -s $(YML_PIDCALIB_EFFS) \
		-y 2017 2>&1 | tee $(OUT_DIR)/stdout.log
...
build-rdx-misid-mc-corrections-2018: $(BINPATH)/GetMisIDCorrections
	...
	@mkdir -p $(OUT_DIR)/figs/fits
	$< -o $(OUT_DIR) -c $(YML_FILE) -b $(YML_D0_BKG) -s $(YML_PIDCALIB_EFFS) \
		-y 2018 2>&1 | tee $(OUT_DIR)/stdout.log
...
build-rdx-misid-mc-corrections-2016-run2ang: $(BINPATH)/GetMisIDCorrections
	...
	@mkdir -p $(OUT_DIR)/figs/params
	@mkdir -p $(OUT_DIR)/figs/fits
	$< -o $(OUT_DIR) -c $(YML_FILE_RUN2ANG) -b $(YML_D0_BKG_RUN2ANG) \
		-s $(YML_PIDCALIB_EFFS_RUN2ANG) -y 2016 --run2ang --float_rich \
		2>&1 | tee $(OUT_DIR)/stdout.log
...
build-rdx-misid-mc-corrections-2017-run2ang: $(BINPATH)/GetMisIDCorrections
	...
	@mkdir -p $(OUT_DIR)/figs/fits
	$< -o $(OUT_DIR) -c $(YML_FILE_RUN2ANG) -b $(YML_D0_BKG_RUN2ANG) \
		-s $(YML_PIDCALIB_EFFS_RUN2ANG) -y 2017 --run2ang --float_rich
		2>&1 | tee $(OUT_DIR)/stdout.log
...
build-rdx-misid-mc-corrections-2018-run2ang: $(BINPATH)/GetMisIDCorrections
	...
	@mkdir -p $(OUT_DIR)/figs/fits
	$< -o $(OUT_DIR) -c $(YML_FILE_RUN2ANG) -b $(YML_D0_BKG_RUN2ANG) \
		-s $(YML_PIDCALIB_EFFS_RUN2ANG) -y 2018 --run2ang --float_rich
		2>&1 | tee $(OUT_DIR)/stdout.log
...
build-rdx-misid-mc-corrections-2016-vmu: $(BINPATH)/GetMisIDCorrections
	...
	@mkdir -p $(OUT_DIR)/figs/fits
	$< -o $(OUT_DIR) -c $(YML_FILE) -b $(YML_D0_BKG) -s $(YML_PIDCALIB_EFFS)
		-y 2016 --vmu 2>&1 | tee $(OUT_DIR)/stdout.log
...
build-rdx-misid-mc-corrections-2017-vmu: $(BINPATH)/GetMisIDCorrections
	...
	@mkdir -p $(OUT_DIR)/figs/fits
	$< -o $(OUT_DIR) -c $(YML_FILE) -b $(YML_D0_BKG) -s $(YML_PIDCALIB_EFFS) \
		-y 2017 --vmu 2>&1 | tee $(OUT_DIR)/stdout.log
...
build-rdx-misid-mc-corrections-2018-vmu: $(BINPATH)/GetMisIDCorrections
	...
	@mkdir -p $(OUT_DIR)/figs/fits
	$< -o $(OUT_DIR) -c $(YML_FILE) -b $(YML_D0_BKG) -s $(YML_PIDCALIB_EFFS) \
		-y 2018 --vmu 2>&1 | tee $(OUT_DIR)/stdout.log
...
build-rdx-misid-mc-corrections-2016-vmu-run2ang: $(BINPATH)/GetMisIDCorrections
	...
	@mkdir -p $(OUT_DIR)/figs/fits
	$< -o $(OUT_DIR) -c $(YML_FILE_RUN2ANG) -b $(YML_D0_BKG_RUN2ANG) \
		-s $(YML_PIDCALIB_EFFS_RUN2ANG) -y 2016 --vmu --run2ang
		2>&1 | tee $(OUT_DIR)/stdout.log
...
build-rdx-misid-mc-corrections-2017-vmu-run2ang: $(BINPATH)/GetMisIDCorrections
	...
	@mkdir -p $(OUT_DIR)/figs/fits
	$< -o $(OUT_DIR) -c $(YML_FILE_RUN2ANG) -b $(YML_D0_BKG_RUN2ANG) \
		-s $(YML_PIDCALIB_EFFS_RUN2ANG) -y 2017 --vmu --run2ang
		2>&1 | tee $(OUT_DIR)/stdout.log
...
build-rdx-misid-mc-corrections-2018-vmu-run2ang: $(BINPATH)/GetMisIDCorrections
	...
	@mkdir -p $(OUT_DIR)/figs/fits
	$< -o $(OUT_DIR) -c $(YML_FILE_RUN2ANG) -b $(YML_D0_BKG_RUN2ANG) \
		-s $(YML_PIDCALIB_EFFS_RUN2ANG) -y 2018 --vmu --run2ang
		2>&1 | tee $(OUT_DIR)/stdout.log
...
build-rdx-misid-mc-corrections-2016-fake_mu: $(BINPATH)/GetMisIDCorrections
	...
	@mkdir -p $(OUT_DIR)/figs/fits
	$< -o $(OUT_DIR) -c $(YML_FILE) -b $(YML_D0_BKG) -s $(YML_PIDCALIB_EFFS) \
		-y 2016 --fake_mu 2>&1 | tee $(OUT_DIR)/stdout.log
...
build-rdx-misid-mc-corrections-2017-fake_mu: $(BINPATH)/GetMisIDCorrections
	...
	@mkdir -p $(OUT_DIR)/figs/fits
	$< -o $(OUT_DIR) -c $(YML_FILE) -b $(YML_D0_BKG) -s $(YML_PIDCALIB_EFFS) \
		-y 2017 --fake_mu 2>&1 | tee $(OUT_DIR)/stdout.log
...
build-rdx-misid-mc-corrections-2018-fake_mu: $(BINPATH)/GetMisIDCorrections
	...
	@mkdir -p $(OUT_DIR)/figs/fits
	$< -o $(OUT_DIR) -c $(YML_FILE) -b $(YML_D0_BKG) -s $(YML_PIDCALIB_EFFS) \
		-y 2018 --fake_mu 2>&1 | tee $(OUT_DIR)/stdout.log
...
build-rdx-misid-mc-corrections-2016-fake_mu-run2ang: $(BINPATH)/GetMisIDCorrections
	...
	@mkdir -p $(OUT_DIR)/figs/fits
	$< -o $(OUT_DIR) -c $(YML_FILE_RUN2ANG) -b $(YML_D0_BKG_RUN2ANG) \
		-s $(YML_PIDCALIB_EFFS_RUN2ANG) -y 2016 --fake_mu --run2ang
		2>&1 | tee $(OUT_DIR)/stdout.log
...
build-rdx-misid-mc-corrections-2017-fake_mu-run2ang: $(BINPATH)/GetMisIDCorrections
	...
	@mkdir -p $(OUT_DIR)/figs/fits
	$< -o $(OUT_DIR) -c $(YML_FILE_RUN2ANG) -b $(YML_D0_BKG_RUN2ANG) \
		-s $(YML_PIDCALIB_EFFS_RUN2ANG) -y 2017 --fake_mu --run2ang
		2>&1 | tee $(OUT_DIR)/stdout.log
...
build-rdx-misid-mc-corrections-2018-fake_mu-run2ang: $(BINPATH)/GetMisIDCorrections
	...
	@mkdir -p $(OUT_DIR)/figs/fits
	$< -o $(OUT_DIR) -c $(YML_FILE_RUN2ANG) -b $(YML_D0_BKG_RUN2ANG) \
		-s $(YML_PIDCALIB_EFFS_RUN2ANG) -y 2018 --fake_mu --run2ang
		2>&1 | tee $(OUT_DIR)/stdout.log
...
build-rdx-merged:
	...
	@mkdir -p $(OUT_DIR)
	./scripts/merge_histo.py -c $(YML_FILE) -o $(OUT_DIR) \
		2>&1 | tee $(OUT_DIR)/stdout.log

build-rdx-merged-run2ang:
	$(eval OUT_DIR := $(GENPATH)/rdx-$(TIME_STAMP)-merged-run2ang)
	@mkdir -p $(OUT_DIR)
	./scripts/merge_histo.py -c $(YML_FILE_RUN2ANG) -o $(OUT_DIR) \
		2>&1 | tee $(OUT_DIR)/stdout.log
...
build-rdx-tag:
	...
	@mkdir -p $(OUT_DIR)
	./scripts/build_histo_tagged.py -c $(YML_FILE) -o $(OUT_DIR) \
		2>&1 | tee $(OUT_DIR)/stdout.log

build-rdx-tag-run2ang:
	$(eval OUT_DIR := $(GENPATH)/rdx-$(TIME_STAMP)-tag-run2ang)
	@mkdir -p $(OUT_DIR)
	./scripts/build_histo_tagged.py -c $(YML_FILE_RUN2ANG) -o $(OUT_DIR) \
		--run2ang 2>&1 | tee $(OUT_DIR)/stdout.log
...
build-rdx-unfolded-run2ang: $(BINPATH)/UnfoldMisID
	...
	@mkdir -p $(OUT_DIR)
	$< --iteration 5 -e $(EFFICIENCIES_RUN2ANG) -y $(TAGGED_RUN2ANG) \
		-o $(OUT_DIR) -c $(YML_FILE_RUN2ANG) \
		--outputHisto "unfolded_run2ang.root" 2>&1 | tee $(OUT_DIR)/stdout.log

test-unfold: $(BINPATH)/UnfoldMisID
	$(eval OUT_DIR := $(GENPATH)/rdx-$(TIME_STAMP)-test-unfold)
	@mkdir -p $(OUT_DIR)
	$< -c $(YML_FILE) --dryRun --debug -y $(TAGGED) -e $(EFFICIENCIES) \
...
test-unfold-run2ang: $(BINPATH)/UnfoldMisID
	...
	@mkdir -p $(OUT_DIR)
	$< -c $(YML_FILE_RUN2ANG) --dryRun --debug -y $(TAGGED_RUN2ANG) \
		-e $(EFFICIENCIES_RUN2ANG) -o $(OUT_DIR) \
		--outputHisto "unfolded_run2ang.root" 2>&1 | tee $(OUT_DIR)/stdout.log


# Test of application of misID weights
test-apply-rdx-weights: \
	test-apply-rdx-weights-2016 \
	test-apply-rdx-weights-2017 \
	test-apply-rdx-weights-2018
...
test-apply-rdx-weights-2016: $(BINPATH)/ApplyMisIDWeight \
	...
	$< --debug -a -Y 2016 -i $(word 2, $^) -x $(word 3, $^) \
		--kSmrBrName k_smr --piSmrBrName pi_smr -o $(OUT_DIR)/$(AUX_NTP) \
...
test-apply-rdx-weights-run2ang: \
	test-apply-rdx-weights-2016-run2ang \
	test-apply-rdx-weights-2017-run2ang \
	test-apply-rdx-weights-2018-run2ang
...
test-apply-rdx-weights-2016-run2ang: $(BINPATH)/ApplyMisIDWeight \
	...
	$< --debug -a -Y 2016 -i $(word 2, $^) -x $(word 3, $^) \
		--kSmrBrName k_smr --piSmrBrName pi_smr -o $(OUT_DIR)/$(AUX_NTP) \
		-c $(YML_FILE_RUN2ANG) | tee $(OUT_DIR)/stdout.log
